...
public class Day8
{
    public static void Part1(string input)
    {
        ...
        var lines = input.Split(Environment.NewLine).Select(l => l.Split(" | "))
            .Select(l => SegmentDisplay.Decode(l[0]).Read(l[1])).ToArray();

        Console.WriteLine(lines.Sum(l => l.Count(c => c is 1 or 4 or 7 or 8)));
    }
    ...
    public static void Part2(string input)
    {
        ...
        Console.WriteLine(lines.Select(d => d[0] * 1000 + d[1] * 100 + d[2] * 10 + d[3]).Sum());
    }

    public class SegmentDisplay
    {
        private SegmentDisplay(string[] patternReference)
        {
        ...
        public static SegmentDisplay Decode(string uniquePatterns)
        {
            var patternReference = new string[10];
            var patterns = uniquePatterns.Trim().Split(' ');
            patternReference[1] = patterns.Single(p => p.Length == 2);
            patternReference[4] = patterns.Single(p => p.Length == 4);
            patternReference[7] = patterns.Single(p => p.Length == 3);
            patternReference[8] = patterns.Single(p => p.Length == 7);

            patternReference[3] =
                patterns.Single(p =>
                    patternReference[7].All(p.Contains) &&
                    p.ToCharArray().Count(c => !patternReference[7].Contains(c)) == 2);

            patternReference[9] =
                patterns.Single(p =>
                    patternReference[3].All(p.Contains) &&
                    p.ToCharArray().Count(c => !patternReference[3].Contains(c)) == 1);
            var topLeft = patternReference[9].ToCharArray().Single(c => !patternReference[3].Contains(c));

            patternReference[5] = patterns.Single(p => p.Length == 5 && p.Contains(topLeft));
            patternReference[2] =
                patterns.Single(p => p.Length == 5 && p != patternReference[3] && !p.Contains(topLeft));
            patternReference[6] = patterns.Single(p =>
                p.Length == 6 && p != patternReference[9] && patternReference[5].All(p.Contains));
            patternReference[0] =
                patterns.Single(p => p.Length == 6 && p != patternReference[9] && p != patternReference[6]);
            return new SegmentDisplay(patternReference);
        }

        public int[] Read(string display) =>
            display.Split(' ').Select(d => Enumerable.Range(0, 10).Single(c => PatternReference[c].Length == d.Length && PatternReference[c].ToCharArray().All(d.Contains))).ToArray();
    }
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce";
    ...
    public static string Input = @"cbgefad agc fdega cgdf ecdgfa efgca gaefbd edagbc cg ecafb | cdgafbe cfdg cg gac
